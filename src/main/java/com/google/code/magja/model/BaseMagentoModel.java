...
public abstract class BaseMagentoModel<S> implements Serializable {
			...
			try {
				...
				if (prefix.equals("set")) {
					...
					if (fld.getType().equals(Class.forName("java.lang.Boolean"))) {
						...
						// sometimes the arg could still have in boolean type,
						// so we convert here
						Object arg2;
	...
	/**
	 * Get a specific, in the magento's name format, which is or isn't covered
	 * by the concret model
	 */
	public Object get(String name) {
		if (name != null) {
			if (!name.trim().equals("")) {
				...
				// once we have the attribute name, just invoke the proper 'get'
				// for that to invoke the correspondent model's method
				try {
	...
	public void set(String name, Object value) {
		if (name != null && value != null) {
			...
			if (!name.equals("")) {
				 ...
				 * property if exists, we invoke the setter for this property
				 * instead put it on the map
				 */
