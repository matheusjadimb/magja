...
public class CategoryRemoteServiceImpl extends GeneralServiceImpl<Category> implements CategoryRemoteService {
	...
	public CategoryRemoteServiceImpl(MagentoSoapClient soapClient, RemoteServiceFactory serviceFactory) {
		super(soapClient);
	...
	public int save(Category category, String storeView) throws ServiceException {
		if (category.getId() == null) {
			...
			try {
				Integer id = Integer.parseInt((String) soapClient.callArgs(ResourcePath.CategoryCreate,
						new Object[] { category.getParent().getId(), category.getAllProperties() }));
				if (id > -1) {
					category.setId(id);
					return id;
				} else {
			...
			try {
				Boolean sucessed = (Boolean) soapClient.callArgs(ResourcePath.CategoryUpdate, new Object[] { category.getId(), category.getAllProperties(),
